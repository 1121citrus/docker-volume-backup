...
alias ddocker=/usr/local/bin/docker
...
BACKUP_FILENAME="$(date +"${BACKUP_FILENAME:-%Y%m%dT%H%M%S-backup.tar.gz}")"
...
if [ ! -z "${GPG_PASSPHRASE:-$(cat "${GPG_PASSPHRASE_FILE:-/dev/null}" 2>/dev/null || true)}" ]; then 
...
if [ ! -z "${SCP_HOST}" ]; then
  ...
  SSH_CONFIG="-o StrictHostKeyChecking=no -i '${SSH_KEY_FILE:-/ssh/id_rsa}'"
...
INFLUX_LINE=$(echo "${INFLUX_LINE}" | tr -d '[\r\n[:space:]]')

CREDENTIALS=$(cat "${INFLUXDB_CREDENTIALS_FILE:-/dev/null}" 2>/dev/null || true)
INFLUXDB_CREDENTIALS=${INFLUXDB_CREDENTIALS:-${CREDENTIALS}}

API_TOKEN=$(cat "${INFLUXDB_API_TOKEN_FILE:-/dev/null}" 2>/dev/null || true)
INFLUXDB_API_TOKEN=${INFLUXDB_API_TOKEN:-${API_TOKEN}}
